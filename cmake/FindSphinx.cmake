#
# Original file from k0ekk0ek
# https://github.com/k0ekk0ek/cmake-sphinx/blob/master/cmake/Modules/FindSphinx.cmake
#
...
macro(_Sphinx_find_executable _exe)
  ...
  if(SPHINX_${_uc}_EXECUTABLE)
    ...
    if(NOT TARGET Sphinx::${_exe})
      ...
      set_target_properties(Sphinx::${_exe} PROPERTIES IMPORTED_LOCATION "${SPHINX_${_uc}_EXECUTABLE}")
    endif()
...
if(SPHINX_BUILD_VERSION)
  ...
  if("exhale" IN_LIST Sphinx_FIND_COMPONENTS AND NOT "breathe" IN_LIST Sphinx_FIND_COMPONENTS)
...
function(_Sphinx_generate_confpy _target _cachedir)
  if(NOT TARGET Sphinx::quickstart)
    message(FATAL_ERROR "sphinx-quickstart is not available, needed by sphinx_add_docs for target ${_target}")
  endif()
...
function(_Sphinx_update_confpy _sourcedir _cachedir)
  ...
  file(READ "${_sourcedir}/conf.py" CONFIG_PY)
  string(REGEX REPLACE "project = '[a-zA-Z ]+'" "project = '${SPHINX_PROJECT}'" CONFIG_PY "${CONFIG_PY}")
  string(REGEX REPLACE "copyright = '[a-zA-Z ]+'" "copyright = '${SPHINX_COPYRIGHT}'" CONFIG_PY "${CONFIG_PY}")
  string(REGEX REPLACE "author = '[a-zA-Z ]+'" "author = '${SPHINX_AUTHOR}'" CONFIG_PY "${CONFIG_PY}")
  string(REGEX REPLACE "version = '[a-zA-Z ]+'" "version = '${SPHINX_VERSION}'" CONFIG_PY "${CONFIG_PY}")
  string(REGEX REPLACE "release = '[a-zA-Z ]+'" "release = '${SPHINX_RELEASE}'" CONFIG_PY "${CONFIG_PY}")
...
function(sphinx_add_docs _target)
  ...
  if(_args_BREATHE_PROJECTS)
    if(NOT Sphinx_breathe_FOUND)
      message(FATAL_ERROR "Sphinx extension 'breathe' is not available. Needed by sphinx_add_docs for target ${_target}")
    endif()
  ...
  if(NOT EXISTS "${_sourcedir}/conf.py")
    _Sphinx_generate_confpy(${_target} "${_cachedir}")
  else()
    _Sphinx_update_confpy("${_sourcedir}" "${_cachedir}")
  endif()
